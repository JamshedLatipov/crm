...
export class CdrService {
  ...
  const where: Record<string, unknown> = {};
    if (filter.fromDate && filter.toDate) {
      where.calldate = Between(new Date(filter.fromDate), new Date(filter.toDate));
    ...
    if (filter.disposition) {
      if (Array.isArray(filter.disposition)) where.disposition = In(filter.disposition);
    ...
    if (filter.minDuration) qb.andWhere('c.duration >= :minDuration', { minDuration: filter.minDuration });
    if (filter.maxDuration) qb.andWhere('c.duration <= :maxDuration', { maxDuration: filter.maxDuration });
    if (filter.search) {
      qb.andWhere('(c.src ILIKE :s OR c.dst ILIKE :s OR c.clid ILIKE :s)', { s: `%${filter.search}%` });
    }
