...
export class SeedCommentsDemo1740000000004 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      INSERT INTO comments ("userId", "entityType", "entityId", text, "createdAt", "updatedAt")
      SELECT id, 'lead', '1', 'Demo comment for lead 1', now(), now() FROM users WHERE username = 'john.doe'
      ON CONFLICT DO NOTHING
    `);

    await queryRunner.query(`
      INSERT INTO comments ("userId", "entityType", "entityId", text, "createdAt", "updatedAt")
      SELECT id, 'deal', '1', 'Demo comment for deal 1', now(), now() FROM users WHERE username = 'jane.smith'
      ON CONFLICT DO NOTHING
    `);
  }
