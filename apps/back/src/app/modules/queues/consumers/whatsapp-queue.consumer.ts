...
import { WhatsAppMessage, WhatsAppMessageStatus, WhatsAppMessageType } from '../../messages/entities/whatsapp-message.entity';
import { WhatsAppTemplate } from '../../messages/entities/whatsapp-template.entity';
import { WhatsAppProviderService } from '../../messages/services/whatsapp-provider.service';
...
@Injectable()
export class WhatsAppQueueConsumer implements OnModuleInit, OnModuleDestroy {
  ...
  private async setupConsumer() {
    try {
      ...
      // Setup WhatsApp queue
      await this.channel.assertQueue(QUEUE_NAMES.WHATSAPP, {
        durable: true,
        arguments: {
      ...
      // Start consuming
      await this.channel.consume(QUEUE_NAMES.WHATSAPP, async (msg) => {
        if (!msg) return;
        ...
        try {
          ...
          this.logger.error('Failed to process WhatsApp message', (error as Error).stack);
          this.channel?.nack(msg, false, false);
        }
      });

      this.logger.log('WhatsApp Queue Consumer started');
    } catch (error) {
      this.logger.error('Failed to setup WhatsApp consumer', (error as Error).stack);
    }
  }
  ...
  async onModuleDestroy() {
    try {
      ...
      this.logger.log('WhatsApp Queue Consumer stopped');
    } catch (error) {
      this.logger.error('Error closing WhatsApp consumer', (error as Error).stack);
    }
  }
}
