...
export class AutomationService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    this.interval = setInterval(() => {
      this.processTimeBasedAutomation().catch((e) =>
        this.logger.error('Time-based automation error', e as any)
      );
    }, 60_000);
  ...
  async processAutomationTrigger(context: AutomationContext): Promise<void> {
    try {
      ...
      this.logger.log(
        `Processing ${rules.length} automation rules for trigger ${context.trigger}`
      );
      ...
      for (const rule of rules) {
        try {
          ...
          this.logger.log(
            `Executed automation rule "${rule.name}" for ${context.entityType} ${context.entityId}`
          );
        } catch (error) {
          this.logger.error(
            `Error processing automation rule "${rule.name}":`,
            error
          );
        }
      }
  ...
  private async evaluateCondition(
    ...
    switch (field) {
      ...
      case AutomationCondition.STAGE_NOT_EQUALS:
        fieldValue =
          context.entityType === 'deal' ? (entity as Deal).stageId : null;
        break;
      ...
      case AutomationCondition.AMOUNT_BETWEEN:
        fieldValue =
          context.entityType === 'deal' ? (entity as Deal).amount : null;
        break;
      case AutomationCondition.PROBABILITY_GREATER_THAN:
      case AutomationCondition.PROBABILITY_LESS_THAN:
        fieldValue =
          context.entityType === 'deal' ? (entity as Deal).probability : null;
        break;
      ...
      case AutomationCondition.PRIORITY_NOT_EQUALS:
        fieldValue =
          context.entityType === 'lead' ? (entity as Lead).priority : null;
        break;
      case AutomationCondition.SCORE_GREATER_THAN:
      case AutomationCondition.SCORE_LESS_THAN:
        fieldValue =
          context.entityType === 'lead' ? (entity as Lead).score : null;
        break;
      ...
      case AutomationCondition.UPDATED_WITHIN_HOURS:
        const updatedHours = Math.floor(
          (Date.now() - entity.updatedAt.getTime()) / (1000 * 60 * 60)
        );
  ...
  private async executeChangeStage(
    ...
    if (context.entityType === 'deal') {
      await this.dealsService.moveToStage(
        context.entityId,
        config.stageId,
        context.userId,
        context.userName
      ...
      this.logger.log(
        `Automation: Changed deal ${context.entityId} stage to ${config.stageId}`
      );
    } else {
  ...
  private async executeChangeStatus(
    ...
    if (context.entityType === 'deal') {
      await this.dealsService.updateDeal(
        context.entityId,
      ...
      this.logger.log(
        `Automation: Changed deal ${context.entityId} status to ${config.status}`
      );
    } else if (context.entityType === 'lead') {
      await this.leadService.changeStatus(
      ...
      this.logger.log(
        `Automation: Changed lead ${context.entityId} status to ${config.status}`
      );
    }
  }
   ...
   */
  private async executeUpdateAmount(
    config: any,
    context: AutomationContext
  ): Promise<void> {
    ...
    if (context.entityType === 'deal') {
      ...
      this.logger.log(
        `Automation: Updated deal ${context.entityId} amount to ${config.amount}`
      );
    } else {
   ...
   */
  private async executeUpdateProbability(
    config: any,
    context: AutomationContext
  ): Promise<void> {
    ...
    if (context.entityType === 'deal') {
      await this.dealsService.updateProbability(
        context.entityId,
        config.probability,
        context.userId,
        context.userName
      ...
      this.logger.log(
        `Automation: Updated deal ${context.entityId} probability to ${config.probability}%`
      );
    } else {
  ...
  private async executeUpdateScore(
    ...
    if (context.entityType === 'lead') {
      await this.leadService.scoreLead(
        Number(context.entityId),
        config.score,
        context.userId,
        context.userName
      ...
      this.logger.log(
        `Automation: Updated lead ${context.entityId} score to ${config.score}`
      );
    } else {
  ...
  private async executeAddTags(
    ...
    if (context.entityType === 'deal') {
      ...
      // For now, just log
      this.logger.log(
        `Automation: Add tags ${config.tags.join(', ')} to deal ${
          context.entityId
        }`
      );
    } else if (context.entityType === 'lead') {
      ...
      this.logger.log(
        `Automation: Added tags ${config.tags.join(', ')} to lead ${
          context.entityId
        }`
      );
    }
   ...
   */
  private async executeRemoveTags(
    config: any,
    context: AutomationContext
  ): Promise<void> {
    if (
    ...
    if (context.entityType === 'deal') {
      ...
      // For now, just log
      this.logger.log(
        `Automation: Remove tags ${config.tags.join(', ')} from deal ${
          context.entityId
        }`
      );
    } else if (context.entityType === 'lead') {
      ...
      this.logger.log(
        `Automation: Removed tags ${config.tags.join(', ')} from lead ${
          context.entityId
        }`
      );
    }
  ...
  private async executeSendNotification(
    ...
    this.logger.log(
      `Automation: Send notification "${config.message}" for ${context.entityType} ${context.entityId}`
    );
    ...
    if (context.entityType === 'deal') {
      ...
      await this.leadService.addNote(
        Number(context.entityId),
        `Автоматическое уведомление: ${config.message}`,
        context.userId
      );
    }
  }
  ...
  private async executeCreateTask(
    ...
    try {
      ...
      this.logger.log(
        `Automation: Created task "${task.title}" (ID: ${task.id}) for ${context.entityType} ${context.entityId}`
      );
    } catch (error) {
  ...
  private async executeSetReminder(
    ...
    this.logger.log(
      `Automation: Set reminder for ${reminderDate.toISOString()} for ${
        context.entityType
      } ${context.entityId}`
    );
    ...
    if (context.entityType === 'lead') {
      await this.leadService.scheduleFollowUp(
        Number(context.entityId),
        reminderDate,
        config.note || 'Автоматическое напоминание'
      );
    }
  }
  ...
  private async executeLogActivity(
    ...
    this.logger.log(
      `Automation: Log activity "${config.note}" for ${context.entityType} ${context.entityId}`
    );
    ...
    try {
      ...
      for (const rule of timeBasedRules) {
        try {
          ...
          this.logger.log(
            `Time-based automation rule "${rule.name}" triggered`
          );
          ...
          this.logger.error(
            `Error processing time-based rule "${rule.name}":`,
            error
          );
        }
      }
