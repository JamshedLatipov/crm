...
import { CreateTelegramTemplateDto } from '../../../models/message.models';
...
export class TelegramTemplateFormComponent implements OnInit {
  private readonly fb = inject(FormBuilder);
  private readonly router = inject(Router);
  private readonly route = inject(ActivatedRoute);
  private readonly telegramService = inject(TelegramTemplateService);
  ...
  loadTemplate(id: string) {
    ...
    this.telegramService.getById(id).subscribe({
      next: (template) => {
        ...
        // Если есть mediaUrl, устанавливаем его для отображения
        if (template.mediaUrl) {
          this.uploadedMediaUrl.set(template.mediaUrl);
          // Извлекаем имя файла из URL
          const filename = template.mediaUrl.split('/').pop() || 'file';
      ...
      error: () => {
        ...
        this.loading.set(false);
        this.router.navigate(['/messages/telegram-templates']);
      }
    });
  }
  ...
  save() {
    ...
    const dto: CreateTelegramTemplateDto = {
      name: formValue.name,
    ...
    const operation = this.templateId()
      ? this.telegramService.update(this.templateId()!, dto)
      : this.telegramService.create(dto);
  ...
  onFileSelected(event: Event): void {
    ...
    // Валидация размера (20MB для Telegram)
    const maxSize = 20 * 1024 * 1024;
    if (file.size > maxSize) {
      this.snackBar.open('Файл слишком большой. Максимальный размер: 20MB', 'Закрыть', { duration: 5000 });
      return;
    }
