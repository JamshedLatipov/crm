...
export class AssignLeadDialogComponent {
  ...
  private loadManagers(): void {
    ...
    this.userService.getManagers().subscribe({
      next: (managers) => { this.availableManagers = managers; this.loading = false; },
      error: (error) => { console.error('Error loading managers:', error); this.loading = false; }
    });
  }
  ...
  getCurrentAssigneeName(): string { const id = this.data.currentAssignee; if (!id) return ''; const manager = this.availableManagers.find(m => m.id?.toString() === id?.toString()); return manager?.fullName || id; }

  assignLead(): void {
    if (!this.isAssignmentValid()) return; this.loading = true; const type = this.assignForm.get('assignmentType')?.value; let assigneeId: string;
    if (type === 'single') { if (this.selectedManager) assigneeId = this.selectedManager.id.toString(); else { this.loading = false; return; } }
    else if (type === 'team') assigneeId = this.selectedTeamMembers[0];
