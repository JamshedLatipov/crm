...
@Component({
  ...
  template: `
    <div class="p-4 space-y-4 text-sm">
      <h1 class="text-xl font-semibold flex items-center gap-3 flex-wrap">
        IVR Admin
        <button
        ...
        <button
          ...
          (click)="toggleReorder()"
        >
          {{ reorderMode() ? 'Done Reorder' : 'Reorder' }}
        </button>
        <button
      ...
      <div class="flex gap-4">
        <div class="w-1/3 border rounded p-2 h-[75vh] overflow-auto space-y-2">
          ...
          >
            <li
              *ngFor="let n of rootNodes(); let i = index"
              cdkDrag
              (click)="select(n)"
              (keyup.enter)="select(n)"
              tabindex="0"
              [class.font-bold]="selected()?.id === n.id"
              class="cursor-pointer hover:bg-blue-50 px-1 rounded outline-none focus:ring flex items-center gap-1"
              [class.bg-yellow-50]="reorderMode()"
            >
              <span
                *ngIf="reorderMode()"
              ...
              >
              <span>{{ i + 1 }}.</span>
              <span>{{ n.name }} ({{ n.action }})</span>
            </li>
          </ul>
          <div *ngIf="selectedRootChildren().length" class="mt-4">
            <div
              class="text-xs uppercase text-gray-500 flex items-center justify-between"
            >
              Children
              <span
                *ngIf="digitConflicts().length"
                class="text-red-600 font-medium"
                title="Duplicate digits"
                >Conflicts: {{ digitConflicts().join(',') }}</span
              >
            </div>
            <ul
              cdkDropList
            ...
            >
              <li
                *ngFor="let c of selectedRootChildren(); let ci = index"
                cdkDrag
                (click)="select(c); $event.stopPropagation()"
                (keyup.enter)="select(c); $event.stopPropagation()"
                tabindex="0"
                class="cursor-pointer hover:bg-blue-50 px-1 rounded outline-none focus:ring flex items-center gap-1"
                [class.bg-yellow-50]="reorderMode()"
                [class.text-red-600]="conflictDigitsSet().has(c.digit || '')"
              >
                <span
                  *ngIf="reorderMode()"
                  class="cursor-move text-gray-400"
        ...
        <div class="flex-1 space-y-4">
          <form
            *ngIf="form"
            [formGroup]="form"
            (ngSubmit)="save()"
            class="grid gap-2 grid-cols-2 border rounded p-3 bg-gray-50"
          >
            <div class="col-span-2 font-medium" aria-label="Node Editor">
              Node Editor
              <span *ngIf="hasErrors()" class="text-red-600 ml-2"
                >(Fix validation errors)</span
              >
            </div>
            ...
            <div>
              ...
              <input
                ...
                maxlength="1"
              />
              <div class="text-[11px] text-red-600" *ngIf="errors()['digit']">
                {{ errors()['digit'] }}
              </div>
            </div>
            <div>
            ...
            <div class="col-span-2">
              <div class="flex items-center justify-between">
                ...
                <small class="text-[11px] text-gray-500"
                  >Select uploaded sound or enter custom payload</small
                >
              </div>
              <div class="flex gap-2 mt-1">
                <select
                  ...
                  [value]="form.value.payload"
                  (change)="onMediaSelect(mediaSel.value)"
                >
                  <option value="">-- none --</option>
                  <option
                    *ngFor="let m of mediaList()"
                    [value]="mediaPayload(m.filename)"
                  >
                    {{ m.name }}
                  </option>
                </select>
              ...
              <div class="text-[11px] text-red-600" *ngIf="errors()['payload']">
                {{ errors()['payload'] }}
              </div>
            </div>
            <div *ngIf="form?.value.action === 'queue'" class="col-span-2">
              <span class="text-xs uppercase">Target Queue Name</span>
              <input
                class="w-full border rounded px-2 py-1"
              ...
              <div class="text-[11px] text-gray-500">
                Enter the queue name (must match Asterisk queues table)
              </div>
            </div>
            <div>
            ...
            <div class="col-span-2 flex items-center gap-2 mt-1">
              <input
                type="checkbox"
                formControlName="allowEarlyDtmf"
                id="allowEarlyDtmf"
              />
              <label for="allowEarlyDtmf" class="text-xs uppercase"
                >Allow Early DTMF During Prompt</label
              >
            </div>
            <div class="col-span-1">
          ...
          <div *ngIf="form" class="border rounded p-3">
            ...
            <div *ngIf="previewChildren().length" class="text-xs">
              ...
              <div
                *ngFor="let c of previewChildren()"
                class="flex gap-2"
                [class.text-red-600]="conflictDigitsSet().has(c.digit || '')"
              >
          ...
          <div *ngIf="!form" class="text-gray-500">
...
export class IvrAdminComponent {
  ...
  breadcrumb = computed<IvrNodeDto[]>(() => {
    ...
    const map = new Map(
      all.filter((n) => n.id).map((n) => [n.id as string, n] as const)
    );
  ...
  conflictDigitsSet = computed(() => {
    ...
    siblings.forEach((s) => {
      if (s.digit) counts[s.digit] = (counts[s.digit] || 0) + 1;
    });
  ...
  reload() {
    this.api.roots().subscribe((rs) => {
      ...
      filtered.forEach((r) => {
        if (r.id) this.loadChildren(r.id);
      });
      // load media
      this.api.mediaList().subscribe((m) => this.mediaList.set(m || []));
    });
  ...
  save() {
    ...
    if (val.id) {
      this.api.update(val.id, val).subscribe((n) => {
        this.reload();
        const sel =
          this.rootNodes().find((r) => r.id === n.id) ||
          Object.values(this.childrenMap())
            .flat()
            .find((c) => c.id === n.id);
  ...
  dropRoot(ev: CdkDragDrop<IvrNodeDto[]>) {
    ...
    arr.forEach(
      (n) => n.id && this.api.update(n.id, { order: n.order }).subscribe()
    );
  }
  ...
  dropChildren(ev: CdkDragDrop<IvrNodeDto[]>) {
    ...
    if (sel?.id) {
      this.childrenMap.update((m) => ({ ...m, [sel.id as string]: sib }));
    }
    sib.forEach(
      (n) => n.id && this.api.update(n.id, { order: n.order }).subscribe()
    );
  ...
  validate() {
    ...
    if (
      action &&
      ['playback', 'dial', 'goto'].includes(action) &&
      !v['payload']
    ) {
      errs['payload'] = 'Payload required for ' + action;
    }
    if (
      action === 'menu' &&
      (v['timeoutMs'] == null || (v['timeoutMs'] as number) <= 0)
    )
      errs['timeoutMs'] = 'Timeout required for menu';
    if (v['parentId'] && !v['digit'])
      errs['digit'] = 'Digit required for child node';
    if (v['digit'] && !/^[0-9*#]$/.test(v['digit'] as string))
      errs['digit'] = 'Digit must be one of 0-9 * #';
    if (this.conflictDigitsSet().has((v['digit'] as string) || ''))
      errs['digit'] = 'Digit conflicts with sibling';
    ...
    if (action === 'menu') {
      ...
      const childDigits = new Set(
        children.map((c) => c.digit).filter(Boolean) as string[]
      );
      (['backDigit', 'repeatDigit', 'rootDigit'] as const).forEach((k) => {
        const val = v[k];
