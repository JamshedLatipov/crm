...
export interface AutomationConditionRule {
  ...
  value2?: string | number; // Для between оператора
}

export interface AutomationActionRule {
  type: AutomationAction;
  config: Record<string, unknown>; // Конфигурация действия (зависит от типа)
}
...
export interface AutomationRule {
  ...
  isActive: boolean;
  priority: number; // Порядок выполнения (меньше число = выше приоритет)
  createdAt: Date;
  updatedAt: Date;
  createdBy?: string;
