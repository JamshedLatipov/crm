    ...
    <div class="p-4">
      ...
      >
        <div style="display:flex; gap:8px; align-items:center">
          ...
          @if (isNew) {
            <div style="display:flex; gap:8px; align-items:center">
              <label style="margin-right:8px">Название:</label>
              <input [(ngModel)]="campaign.name" placeholder="Название кампании" />
            </div>
          }
          @if (!isNew) {
            <h2 style="margin:0">Кампания: {{ campaign?.name }}</h2>
          }
        </div>
        <div>
          <button
      ...
      <div class="mb-4">
        <label>Бюджет: </label>
        <input [(ngModel)]="budget" type="number" aria-label="Бюджет" />
        <button mat-flat-button color="primary" (click)="saveBudget()">
          Сохранить
        </button>
      </div>

      <h3>Метрики (последние {{ metrics?.length }} дней)</h3>
      <div *ngIf="metrics?.length">
        <canvas #chartCanvas width="600" height="200"></canvas>
      </div>
      <div *ngIf="!metrics?.length">Нет метрик для отображения</div>
    </div>
  `,
})
export class AdsCampaignDetailComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.isNew = rawId === 'new' || Number.isNaN(id);

    if (this.isNew) {
      this.campaign = { name: '', status: 'draft', budget: null };
      this.budget = null;
      return;
    }
    ...
    try {
      ...
      this.chart = new Chart(ctx, {
        ...
        data: {
          labels,
          datasets: [
            { label: 'Leads', data, borderColor: '#3b82f6', fill: false },
          ],
        },
        options: {
          responsive: true,
  ...
  saveBudget() {
    ...
    if (this.isNew) {
      const payload: Partial<any> = { name: this.campaign.name, budget: this.budget, status: this.campaign.status };
      this.ads.createCampaign(payload).subscribe((r) => {
        if (r.success && r.data) {
          this.router.navigate(['/ads', String(r.data.id)]);
        }
      });
      return;
    }
  ...
  pause() {
    if (this.campaign)
      this.ads
        .pauseCampaign(this.campaign.id)
        .subscribe((r) => (this.campaign = r.data));
  }
  resume() {
    if (this.campaign)
      this.ads
        .resumeCampaign(this.campaign.id)
        .subscribe((r) => (this.campaign = r.data));
  }
}
