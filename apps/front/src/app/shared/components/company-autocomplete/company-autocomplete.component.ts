...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, MatFormFieldModule, MatInputModule, MatAutocompleteModule, MatIconModule, MatButtonModule, MatOptionModule],
  template: `
    <mat-form-field appearance="outline" class="full-width">
      <mat-label>{{ placeholder || 'Компания' }}</mat-label>
      <input type="text" matInput [matAutocomplete]="auto" [formControl]="control" autocomplete="off" />
      ...
      <div class="create-inline" *ngIf="showCreateInline">
        <button mat-mini-button color="primary" (click)="createCompanyFromInput()">Создать компанию «{{ inlineCompanyText }}»</button>
      </div>
    </mat-form-field>
  `,
  styles: [
    `
      ...
      .create-inline { margin-top: 6px; }
    `
  ]
})
export class CompanyAutocompleteComponent implements OnInit {
  @Input({ required: true }) control!: import('@angular/forms').FormControl<unknown>;
  @Input() idControl?: import('@angular/forms').FormControl<string | null> | undefined;
  ...
  constructor() {
    // nothing here — valueChanges wiring done in ngOnInit via setter below
  }
  ...
  vc.valueChanges.pipe(
      debounceTime(300),
      distinctUntilChanged(),
      switchMap((value: unknown) => {
        const q = typeof value === 'string' ? value : ((value as Company)?.name ?? (value as Company)?.legalName) ?? '';
        ...
        return this.companiesService.searchCompanies(q as string).pipe(catchError(() => of([] as Company[])));
      })
    ).subscribe((list: Company[]) => {
      this.companyOptions = list || [];
      this.showCreateInline = (this.companyOptions.length === 0 && !!this.inlineCompanyText);
    });
  }

  onSelected(company: Company | null) {
    if (!company) return;
  if (this.control) this.control.setValue(company.name ?? company.legalName ?? null);
  ...
  createCompanyFromInput() {
    ...
    ref.afterClosed().subscribe((created: Company | null | undefined) => {
      if (!created) return;
      if (this.control) this.control.setValue(created.name ?? created.legalName ?? null);
